    ...
    <aside className="w-72 h-[calc(100vh-5rem)] fixed left-0 top-20 z-30 flex flex-col glass-panel border-r border-white/10 bg-[#020204]/80 backdrop-blur-2xl">
      <div className="flex-1 px-4 py-8 flex flex-col gap-3">
          ...
          {items.map(item => {
            ...
            return (
              <button
                ...
                className={`
                  w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-200 text-base font-bold border
                  ${isActive 
                    ? 'bg-spark-accent/20 text-white shadow-[0_0_20px_rgba(59,130,246,0.15)] border-white/10' 
                    : 'border-transparent text-white/60 hover:text-white hover:bg-white/5'}
                `}
              >
                <Icon size={20} className={isActive ? 'text-spark-accent' : 'text-white/40 group-hover:text-white'} /> {item.label}
              </button>
            )
          })}
        </div>
        <button 
          ...
          className="w-full flex items-center justify-center gap-3 px-4 py-4 rounded-xl font-black text-base transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] bg-gradient-to-r from-spark-accent via-spark-accent to-indigo-600 text-white shadow-[0_8px_20px_rgba(59,130,246,0.3)] border border-white/20"
        >
          <PenTool size={20} /> {t('prompt_nav_create')}
        </button>
      </div>
    </aside>
  );
